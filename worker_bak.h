...
void worker_load_file(int workerID,int ref_file_num,int ref_file_size,int ref_ignore)
{
	int offset = ref_file_size;
	int load_file_num = ref_file_num / worker_N;

#pragma omp parallel for
	for(int i = 0; i < load_file_num; ++i)
	{
		if(readDataFile(ref_table[workerID * load_file_num  + i],h_ref_ra + i * offset,h_ref_dec + i * offset,offset,ref_ignore) == -1)
			printf("load file %s error!\n",ref_table[workerID * load_file_num + i]);
	}
	return;
}
...
#pragma omp parallel for
		...
		//		MPI_Recv(h_S_cnt,cntSize,MPI_INT,MASTER_NODE,3,MPI_COMM_WORLD,&status);

		long long sum = 0;
		for(int i = 0; i < cntSize; ++i)
			//		sum += h_R_cnt[i] * h_S_cnt[i];
			sum += h_R_cnt[i];
...
void singleCM(int rank,double *h_ref_ra,double *h_ref_dec,int *h_ref_pix,double *h_sam_ra,double *h_sam_dec,int *h_sam_pix,int ref_N,int sam_N,int *h_sam_match,int *h_sam_matchedCnt)
{
	...
	int factor = 20;
	int part_sam_N = 65536;
	int part_ref_N = factor * part_sam_N;
...
#pragma omp atomic
			...
			int gpu_ite = (cur_ref_N - 1) / part_ref_N + 1;
			for(int gpu_i = 0; gpu_i < gpu_ite; ++gpu_i)
			{
				int gpu_ref_N;
				if(gpu_i == gpu_ite - 1)
					gpu_ref_N = cur_ref_N - part_ref_N * gpu_i;
				else
					gpu_ref_N = part_ref_N;

				checkCudaErrors(cudaMemcpy(d_ref_ra[i],h_ref_ra + start_ref_pos + gpu_i * part_ref_N,gpu_ref_N * sizeof(double),cudaMemcpyHostToDevice));
				checkCudaErrors(cudaMemcpy(d_ref_dec[i],h_ref_dec + start_ref_pos + gpu_i * part_ref_N,gpu_ref_N * sizeof(double),cudaMemcpyHostToDevice));
				checkCudaErrors(cudaMemcpy(d_ref_pix[i],h_ref_pix + start_ref_pos + gpu_i * part_ref_N,gpu_ref_N * sizeof(int),cudaMemcpyHostToDevice));
				kernel_singleCM<<<grid,block>>>(d_ref_ra[i],d_ref_dec[i],d_ref_pix[i],gpu_ref_N,d_sam_ra[i],d_sam_dec[i],d_sam_pix[i],cur_sam_N,d_sam_match[i],d_sam_matchedCnt[i],start_ref_pos,start_sam_pos);
...
void worker_ownCM(int rank)
{
	...
	if(worker_sam_N == 0 || worker_ref_N == 0)
	{
		printf("rank-%d no reference point, return directly\n",rank);
	}
	int *ref_match,*sam_match;
	int *ref_matchedCnt,*sam_matchedCnt;
	ref_match = (int*)malloc(sizeof(int) * worker_ref_N * 5);
	ref_matchedCnt = (int*)malloc(sizeof(int) * worker_ref_N);
...
void worker_CM(int rank)
{
	...
	for(int i = 0; i < worker_N - 1; ++i)
	{
		...
		{
			if(send_amount && ((kk == 0 && commID > rank) || (kk == 1 && commID < rank)))
			{
			...
			else if(worker_ref_N && ((kk == 0 && commID < rank) || (kk == 1 && commID > rank)))
			{
